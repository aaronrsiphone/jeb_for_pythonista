...
This is a template for the global `JEB.md`. Copy it to `~/miniagent/JEB.md`;
that file is automatically discovered by MiniAgent at startup and its
contents are injected into the system prompt for every project on this
install. A project can also provide its own `JEB.md` in the workspace root;
that local content is appended after the global content.
